...
const URLTranding =
  "https://api.themoviedb.org/3/trending/movie/week?api_key=c6a36b80cc4c15edf75aebb21bb21aa9";
...
const Tranding = (movies) => {
  let HTMLtext = "";
  const container_movies = document.querySelector(
    ".movies_tranding .container_movies"
  );
  for (let z = 0; z < 8; z++) {
    const data = movies[z];
    const year = new Date(data.release_date).getFullYear();
...
const detailMovie = (id) => {
  let movie = "";
  fetch(
    `https://api.themoviedb.org/3/movie/${id}?api_key=c6a36b80cc4c15edf75aebb21bb21aa9&language=en-US`
  )
    .then((res) => res.json())
    .then((data) => {
      let HTMLCard = "";
...
const nav_tranding = document.getElementById("nav_tranding");
const nav_about = document.getElementById("nav_about");
const nav = document.querySelector("nav");
nav_home.addEventListener("click", () => {
  nav.classList.remove("open");
});
nav_tranding.addEventListener("click", () => {
  nav.classList.remove("open");
});
